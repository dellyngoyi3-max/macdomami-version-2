...
function setDeliveryFieldsVisible() {
  const mode = $("orderMode").value;
  const isDelivery = mode === "delivery";
  const isDineIn = mode === "dinein";

  ["communeLabel", "addressLabel", "landmarkLabel", "precisionLabel", "contact2Label"].forEach((id) => {
  ...
  if (tableLabel) {
    tableLabel.style.display = isDineIn ? "grid" : "none";
    const tableInput = $("tableInput");
    if (tableInput && !tableInput.value && getActiveTable()) {
      tableInput.value = getActiveTable();
    }
  }

  const timeLabel = $("timeLabel");
  if (timeLabel) {
    timeLabel.style.display = isDelivery || isDineIn ? "grid" : "none";
    const txt = isDelivery
      ? "Heure de livraison souhaitée (optionnel)"
      : isDineIn
      ? "Heure de service souhaitée (optionnel)"
      : "Heure de retrait souhaitée (optionnel)";
    timeLabel.childNodes[0].textContent = txt;
  }
}
...
function submitOrder(e) {
  ...
  const table = isDineIn ? ($("tableInput") ? $("tableInput").value.trim() : getActiveTable() || "") : "";
  ...
  if (isDineIn && !table) {
    ...
    showToast("Veuillez préciser votre numéro de table.");
    return;
  }
...
function renderConfirm(order) {
  ...
  } else if (order.mode === "dinein") {
    loc.push(`<div class="confirm__line"><span>Service</span><strong>Sur place — Table n° ${esc(order.table || "")}</strong></div>`);
  } else {
  ...
  $("confirmBody").innerHTML = `
    ...
    <div class="pay-hint">${order.payment.includes("Cash") ? "Règlement en espèces auprès de notre équipe." : "Aucun paiement en ligne : après confirmation, Mami vous enverra le numéro Mobile Money pour régler."}</div>
...
function buildWhatsAppMessage(order) {
  const m = [];
  if (order.mode === "dinein" && order.table) {
    m.push(`*COMMANDE SUR PLACE — TABLE N° ${order.table}*`);
  } else {
  ...
  if (order.mode === "delivery") {
    ...
    m.push(`Mode : Sur place (Table n° ${order.table || "à préciser"})`);
  } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Si une table est active (scan QR code)
  if (getActiveTable()) {
    $("orderMode").value = "dinein";
    $("zoneField").style.display = "none";
    if ($("tableInput")) $("tableInput").value = getActiveTable();
  }

  window.onTableChange = (t) => {
    if (!t) {
      $("orderMode").value = "delivery";
      $("zoneField").style.display = "grid";
      if ($("tableInput")) $("tableInput").value = "";
    } else {
      $("orderMode").value = "dinein";
      $("zoneField").style.display = "none";
      if ($("tableInput")) $("tableInput").value = t;
    }
    renderCart();
  };
